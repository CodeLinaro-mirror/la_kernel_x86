...
static irqreturn_t pn54x_dev_irq_handler(int irq, void *dev_id)
{
	...
	wake_lock_timeout(&pn54x_dev->read_wake, 1*HZ);
...
static int pn54x_dev_release(struct inode *inode, struct file *filp)
{
	...
	if (wake_lock_active(&pn54x_dev->read_wake))
		wake_unlock(&pn54x_dev->read_wake);
...
#endif
{
	...
	ret = gpio_request(pdata->ven_gpio, NFC_ENABLE_GPIO);
	if (ret){
	...
	if (gpio_is_valid(pdata->firm_gpio)) {
		...
		ret = gpio_request(pdata->firm_gpio, NFC_FW_RESET_GPIO);
		if (ret){
	...
	if (gpio_is_valid(pdata->clkreq_gpio)) {
		...
		ret = gpio_request(pdata->clkreq_gpio, NFC_CLKREQ_GPIO);
		if (ret){
	...
	wake_lock_init(&pn54x_dev->read_wake, WAKE_LOCK_SUSPEND, "pn544_nfc");
...
err_misc_register:
	wake_lock_destroy(&pn54x_dev->read_wake);
	kfree(pn54x_dev);
err_exit:
	if (gpio_is_valid(pdata->clkreq_gpio))
