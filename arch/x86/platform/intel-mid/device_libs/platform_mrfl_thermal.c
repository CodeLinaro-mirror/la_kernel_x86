...
void *mrfl_thermal_platform_data(void *info)
{
	...
	if (INTEL_MID_BOARD(2, PHONE, MRFL, BB, ENG) ||
		...
		return &pdata[bdgb_thermal];
	else
		return &pdata[mrfl_thermal];
}

static const struct devs_id bcove_thrm_dev_id __initconst = {
	.name = "bcove_thrm",
