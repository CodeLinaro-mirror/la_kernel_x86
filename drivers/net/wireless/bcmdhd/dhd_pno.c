...
dhd_pno_clean(dhd_pub_t *dhd)
{
	...
	DHD_PNO(("%s enter\n", __FUNCTION__));
	/* Disable PNO */
	err = dhd_iovar(dhd, 0, "pfn", (char *)&pfn, sizeof(pfn),
		NULL, 0, TRUE);
	if (err < 0) {
...
_dhd_pno_enable(dhd_pub_t *dhd, int enable)
{
	...
	/* Enable/Disable PNO */
	err = dhd_iovar(dhd, 0, "pfn", (char *)&enable, sizeof(enable),
		NULL, 0, TRUE);
	if (err < 0) {
...
_dhd_pno_set(dhd_pub_t *dhd, const dhd_pno_params_t *pno_params, dhd_pno_mode_t mode)
{
	...
	if (mode == DHD_PNO_BATCH_MODE) {
		...
		err = dhd_iovar(dhd, 0, "pfnmem", NULL, 0,
			(char *)&_tmp, sizeof(_tmp), FALSE);
		if (err < 0) {
...
_dhd_pno_cfg(dhd_pub_t *dhd, uint16 *channel_list, int nchan)
{
	...
	err = dhd_iovar(dhd, 0, "pfn_cfg", (char *)&pfncfg_param, sizeof(pfncfg_param),
		NULL, 0, TRUE);
	if (err < 0) {
