	...
	case ST21NFC_RECOVERY:
		...
		if (st21nfc_dev->platform_data.reset_gpio != 0) {
			...
			gpio_set_value(st21nfc_dev->platform_data.reset_gpio,
				       0);
			msleep(10);
			/* during the reset, force IRQ OUT as DH output instead */
			/* of input in normal usage */
			ret =
			    gpio_direction_output(st21nfc_dev->
						  platform_data.irq_gpio, 1);
			if (ret) {
...
static int nfc_parse_dt(struct device *dev,
			struct st21nfc_i2c_platform_data *pdata)
{
	int r = 0;
